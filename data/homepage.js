...
if (!DataStore.has("Wallpaper-list")) {
	DataStore.set("Wallpaper-list",["Ryaks1.webm","Ryaks2.webm"])
}
...
if (DataStore.get("Dev-mode")) {
	let res = await fetch(`${path}RyaksV3-Data/data/configs/Language.js`)
	if (res.status == 200) {
		lang = (await (() => import(`${path}RyaksV3-Data/data/configs/Language.js`))()).default
	}
...
function go_to_default_home_page() {
	document.querySelector(`lol-uikit-navigation-item[item-id='Ryaks-home']`).click()
}
...
function loadBG(BG) {
	let RyaksBg = document.getElementById("Ryaks-bg")
	RyaksBg.src = `${datapath}assets/Backgrounds/Wallpapers/${BG}`
}
...
else {console.log(eConsole+"%c Running %cRyaks theme - %cStable %cversion",eCss,"","color: #e4c2b3","color: #00ff44","")}
if (DataStore.get("Continues_Audio")) {
